...
class UsersModel {
  ...
  async show(id: number): Promise<User> {
    try {
      ...
      throw new Error(`Could not retrieve user with id ${id}: ${err}`)
    }
  }
  ...
  async create(u: User): Promise<User> {
    try {
      ...
      throw new Error(`Could not add user ${u.user_name}. Error: ${err}`)
    }
  }
  ...
  async edit(u: User): Promise<User> {
    try {
      ...
      throw new Error(`Could not update user with id ${u.id}: ${err}`)
    }
  }
